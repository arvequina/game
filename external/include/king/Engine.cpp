...
namespace King {
	...
	void Engine::SetMouseButtonUp(bool state) {
	...
	void Engine::fillScene() {
		...
		const float pos_x_end = 645.0f;
		const float pos_increment = 43.0f;
		int row = 0;
		// make it random at some point
		for (float pos = pos_x_ini; pos < pos_x_end; pos += pos_increment) {
			Render(King::Engine::TEXTURE_GREEN, pos, pos_y_ini + row * pos_increment);
		}
		++row;
		for (float pos = pos_x_ini; pos < pos_x_end; pos += pos_increment) {
			Render(King::Engine::TEXTURE_YELLOW, pos, pos_y_ini + row * pos_increment);
		}
		++row;
		for (float pos = pos_x_ini; pos < pos_x_end; pos += pos_increment) {
			Render(King::Engine::TEXTURE_RED, pos, pos_y_ini + row * pos_increment);
		}
		++row;
		for (float pos = pos_x_ini; pos < pos_x_end; pos += pos_increment) {
			Render(King::Engine::TEXTURE_BLUE, pos, pos_y_ini + row * pos_increment);
		}
		++row;
		for (float pos = pos_x_ini; pos < pos_x_end; pos += pos_increment) {
			Render(King::Engine::TEXTURE_GREEN, pos, pos_y_ini + row * pos_increment);
